...
col = Atriplets.col
data = Atriplets.data
Abuilder = ti.SparseMatrixBuilder(N,N, max_num_triplets=2500000)
@ti.kernel
def buildA(Abuilder: ti.sparse_matrix_builder(), row: ti.ext_arr(), col: ti.ext_arr(), data: ti.ext_arr()):
    for i in range(nnz):
